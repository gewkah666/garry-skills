...
做什么：同步镜像 → 聚合上月（收入 / 支出 / 结余、环比、一级二级分类、每日与累计、商户 Top 10、大额 Top 10、账户月初月末余额、30 天未对账账户）→ 渲染 `templates/monthly.html.tmpl`（在 plugin 仓库里）→ 上传 dashboards → 写阅览世界一页（类型=财务，标签 财务 / 月报，链接指向报告）→ `phantom reach.notify L1 kind=finance` 飞书一句话带链接 → `scripts/feishu_file.py` 把 HTML 文件本身也发到飞书（dashboards 链接在 Phantom 门后面，只有 Phantom App 能打开；飞书里点文件用浏览器看）。HTML 副本留在 `~/.hermes/finance-reports/<月> 财务月报.html`。
